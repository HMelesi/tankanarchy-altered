...
class Drawing {
  ...
  static create(canvas, viewport) {
    ...
    for (const type of Constants.POWERUP_KEYS) {
      ...
      images[
        type
      ].src = `${Constants.DRAWING_IMG_BASE_PATH}/${type}_powerup.png`;
    }
  ...
  drawTank(isSelf, player) {
    ...
    this.drawCenteredImage(
      this.images[
        // eslint-disable-next-line multiline-ternary
        isSelf
          ? Constants.DRAWING_IMG_SELF_TURRET
  ...
  drawTiles() {
    const start = this.viewport.toCanvas({
      x: Constants.WORLD_MIN,
      y: Constants.WORLD_MIN
    });
    const end = this.viewport.toCanvas({
      x: Constants.WORLD_MAX,
      y: Constants.WORLD_MAX
    });
